...
| `--compare-baseline <path>` | (none) | After the bench completes, automatically compare the result against the given baseline snapshot file and print a Markdown report; equivalent to running `agr compare-baseline --current <path>` afterward but in a single command. |
...
| `--stats` | `false` | Show a compact token and cost statistics summary: total/avg/max tokens per step, prompt cache hit rate, and total cost. `--json` emits `{totalSteps, totalIn, totalOut, totalCached, avgIn, avgOut, maxIn, maxOut, cacheHitPct, totalCostUsd}`. |
...
agr status --grid                     # cross-tab PASS/FAIL matrix (test cases x configs)
...
| `--by-matrix` | `false` | Show a per-matrix-sweep breakdown: total runs, solve rate, avg cost, and avg duration per `matrixId`, sorted newest-first. Useful for tracking solve rate trends across multiple `agr bench --matrix` runs. |
